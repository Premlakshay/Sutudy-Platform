...
export default function Home({params}: {
    ...
    const handleCreate = async() => {
        if(cName == "" || admin == "" || price == ""){
            ...
            setDis(true);
            const req = await fetch('/api/admin/it/sub', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
                body: JSON.stringify({_id: cId, name: cName, email: admin, price: price})
              })
    ...
    async function loadCourses(){
        const res: any = await fetch('/api/admin/it/sub', {
            method: 'GET',
        });
                        ...
                        <DialogFooter>
                          <Button disabled={dis} onClick={async()=>{
                            if(admin == "" || cName == ""){
                              ...
                              setDis(true)
                            const createCourse = await fetch('/api/admin/it/sub', {
                            method: 'PUT',
                            headers: {
                              'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({_id: course._id, name: cName, email: admin, price: price}),
                            })
