...
export default function Home({params}: {params: {id: String}}){
    ...
    const [passing, setPassing] = React.useState('');
    const loadQuestion = async() => {
        const req = await fetch('/api/question/exams', {
            method: 'PUT',
            headers: {
        ...
        if(data.isPrevious){
            setPassing(data.isPrevious.passMarks);
    ...
    const save = async() => {
        if(que.length <= 0 || passing == ""){
            ...
            const req = await fetch('/api/question/exams', {
                ...
                body: JSON.stringify({id: cId, questions: que, passMarks: passing}),
            });
            const data = await req.json();
            toast(data.message);
            loadQuestion();
