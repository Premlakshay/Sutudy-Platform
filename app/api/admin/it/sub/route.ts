...
export async function GET(req: NextRequest){
  await connectDB();
  const coureses = await subCourses.find({it: true});
  if(!coureses){
...
export async function PUT(req: NextRequest){
  await connectDB();
  const {_id, email, name, price} = await req.json();
  await subCourses.findByIdAndUpdate(_id, {email, name, price, cer: false, it: true});
  return NextResponse.json({message: "Updated"});
}
export async function POST(req: NextRequest) {
  await connectDB();
  const {_id, name, email, price}: any = await req.json();
  const chekcCourse = await Courses.findById(_id);
  if(!chekcCourse){
    return NextResponse.json({message: "In-valid"})
  }else{
    const newCourse = new subCourses({cId: _id, name: name, email: email, price: price, cer: false, it: true});
